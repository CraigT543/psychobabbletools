 ...
 * Modifications on 5/2/2018 by Craig Tucker <craig@craigtuckerlcsw.com>
 */
...
function openRequest(postid, type) {
 ...
 if (type.indexOf('Update Contact and User Information') == 0) {
  document.location.href = 'patient_select.php?postid=' + postid;
 } else
...
while ($v1 || $v2) {
    ...
    if (!$v2 || $v1 && $v1[1]['datetime'] < $v2[1]['datetime']) {
		...
		$chkpt = lookup_pt(text($v1[1]['user']));
		if ($chkpt!= false){
			$ptid = $chkpt;
		}

		echo "  <td>" . text($v1[1]['user']) . "</td>\n";
		echo "  <td style='cursor:pointer;color:blue;' onclick=\"openPatient(". text($ptid) .")\">" .text($ptname) . "</td>\n";
		echo "  <td style='cursor:pointer;color:blue;'";
		echo " onclick=\"openPatient(". text($ptid) .");setTimeout(function(){openRequest(" .
			 "'" . addslashes($postid)      . "'," .
		...
		// Look up the patient in OpenEMR.
		$chkpt = lookup_pt(text($v2[1]['user']));
		if ($chkpt!= false){
			$ptid = $chkpt;
		}		  
		...
		echo "  <td style='cursor:pointer;color:blue;' onclick=\"openPatient(". text($ptid) .")\">" .text($ptname) . "</td>\n";		
		echo "  <td style='cursor:pointer;color:blue;'";
		echo " onclick=\"openPatient(". text($ptid) .");setTimeout(function(){openMessage(" .
			 "'" . addslashes($messageid)      . "'" .
		")}, 2000);\">" . text($v2[1]['datetime']) . "</td>\n";
		echo "  <td>" . text($v2[1]['user'] == $v2[1]['fromuser'] ?
			 xl('Message from patient: '. text($v2[1]['title'])) : xl('Message to patient: '. text($v2[1]['title']))). "</td>\n";
...
function openPatient(ptid) {
 myRestoreSession();
 top.RTop.location = "<?php echo $GLOBALS['webroot']; ?>/interface/patient_file/summary/demographics.php?set_pid=" + ptid;
 return this;
}
