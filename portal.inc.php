...
//Added by Craig Tucker. Look up the OpenEMR patient matching this request for the list_request.php
function lookup_pt($wp_login) {
  if (empty($wp_login)) return false;;
  $ptres = sqlStatement("SELECT pid FROM patient_data WHERE cmsportal_login = ?", array($wp_login));
  if (sqlNumRows($ptres) < 1) return false;
  if (sqlNumRows($ptres) > 1) return false;
  $ptrow = sqlFetchArray($ptres);
  return $ptrow['pid'];
} 
